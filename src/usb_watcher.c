...
static void
prime_pid(uint16_t pid, CFRunLoopRef loop) {
	...
	if(src) CFRunLoopAddSource(loop, src, kCFRunLoopDefaultMode);
	...
	if(matching) {
		...
		io_iterator_t iter;
		/* "IOServiceFirstMatch": fires when a new matching device appears. */
		kern_return_t k = IOServiceAddMatchingNotification(port,
			"IOServiceFirstMatch", matching, noop_match_cb, NULL, &iter);
		if(k == kIOReturnSuccess && iter) noop_match_cb(NULL, iter); /* arm + drain */

		/* "IOServiceTerminate": fires when a matching device is removed.
		 * Need a separate matching dict for the second call; reuse a copy. */
		CFMutableDictionaryRef matching2 = CFDictionaryCreateMutableCopy(
			kCFAllocatorDefault, 0, matching);
		if(matching2) {
			io_iterator_t iter2;
			k = IOServiceAddMatchingNotification(port,
				"IOServiceTerminate", matching2, noop_match_cb, NULL, &iter2);
			if(k == kIOReturnSuccess && iter2) noop_match_cb(NULL, iter2);
			/* matching2 is retained by IOServiceAddMatchingNotification. */
		}
	}
...
watcher_thread_main(void *arg) {
	 ...
	 * whole reboot cycle. */
	prime_pid(DFU_MODE_PID, loop);
	prime_pid(RECOVERY_MODE_PID, loop);
